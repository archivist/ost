...
class MapBrowser extends Component {
  ...
  _showLayers(locations, filters, silent) {
    ...
    this.geojson.eachLayer(layer => {
      let type = layer.feature.properties.entityType
  ...
  _prepareFeature(layer) {
    ...
    if(type === 'toponym') {
      ...
      props.title = props.name === 'Неизвестно' ? props.nearestLocality : props.name
      if(props.name === 'Неизвестно' && props.prisonType) props.title += ' (' + props.prisonType.join(', ') + ')'
    }

    props.stats = this.getLabel('map-documents') + ': ' + props.documents + ', ' + this.getLabel('map-mentions') + ': ' + props.fragments
    let popupContent = `<h3>${props.title}</h3><p class="se-stats">${props.stats}</p>`
