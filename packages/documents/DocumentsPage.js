...
class DocumentsPage extends Component {
  ...
  renderFilters($$) {
    ...
    let documentStateFilter = $$('select').addClass('se-type-filter')
      .ref('stateFilter')
      .on('change', this._onSearch)
      .append($$('option').attr('value', 'all').append(
        this.getLabel('select-document-state')
      ))

    documentStates.forEach(type => {
      let option = $$('option').attr('value', type).append(type)
      let documentState = this.state.filters.documentState
      if(type === documentState) option.attr('selected', 'selected')
      documentStateFilter.append(option)
    })

    filters.push($$('div').addClass('se-select').append(documentStateFilter))

    let directionIcon = this.state.direction === 'desc' ? 'fa-sort-down' : 'fa-sort-up'
  ...
  */
  searchData() {
    ...
    let items = []
    let documentClient = this.context.documentClient
    documentClient.searchDocuments(searchValue, language, dataFilters, options, function(err, docs) {
      if (err) {
        this.setState({
  ...
  */
  _loadData() {
    ...
    documentClient.listDocuments(dataFilters, options, function(err, docs) {
      if (err) {
        this.setState({
